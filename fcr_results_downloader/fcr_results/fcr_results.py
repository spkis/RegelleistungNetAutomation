...
def download_data():
    """Download capacity tender results data in XLSX format for today."""
...
def process_data(file_content, hourly=True):
    ...
    """Process and transform data from an Excel file.
    
    This function reads the Excel data from a given file content, ensures relevant
    columns are of float type, converts the 'DATE_FROM' column to a string, and
    processes each row based on the product name and time ranges. It then
    constructs new rows for each country in the specified time ranges and returns a
    DataFrame with the processed data.
    
    Args:
        file_content (str): The content of the Excel file.
        hourly (bool): If True, process data for each hour within the time range; otherwise, use only
            the start hour.
    
    Returns:
        pd.DataFrame: A DataFrame containing the processed and transformed data.
    """
...
def save_data(df):
    """Saves a DataFrame to a CSV file with a timestamped name."""
    today = datetime.today().strftime('%Y%m%d')
...
def send_to_stream(df, topic):
    """Sends DataFrame rows to a specified Kafka topic."""
...
def main():
    """Download data, process it, save it, and send to stream."""
...
def schedule_daily_run():
    """Schedules and runs the main function daily at 10:00."""
